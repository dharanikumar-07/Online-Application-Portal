...
<body>
		...
		$dis="SELECT * FROM `externalod` WHERE `tutor`='approved' AND `hod`='none'";
...
<?php 
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `externalod` SET `hod`='approved' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `externalod` SET `hod`='declined' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
...
?>
		...
		<a href="hodexternalod.php" id="gb"><button type="button" class="btn btn-secondary" 
