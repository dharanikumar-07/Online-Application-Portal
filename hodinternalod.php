...
<body>
		<h3 style="text-align:center;color:red;padding:2px;"><b><i>Internal Onduty Requests</i></b></h3>
		<div id="s">
		...
		$dis="SELECT * FROM `internalod` WHERE `tutor`='approved' AND `hod`='none'";
...
<?php 
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `internalod` SET `hod`='approved' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `internalod` SET `hod`='declined' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
...
?>
		...
		<a href="hododapplication.php" id="gb"><button type="button" class="btn btn-secondary" 
		data-bs-dismiss="modal" id="buti">Go Back</button></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="hodinternalod.php" id="gb"><button type="button" class="btn btn-secondary" 
