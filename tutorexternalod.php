...
<body>
		...
		$dis="SELECT * FROM `externalod` WHERE `tutor`='none'";
		...
		<input value='Pending'  id='u' type='submit' name='pending'>
		</form></td></tr>";
		}
		}
		else
...
<?php 
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `externalod` SET `tutor`='pending' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
		echo "<script>alert('Onduty added to pending list');</script>";
