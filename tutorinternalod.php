...
<body>
		...
		$dis="SELECT * FROM `internalod` WHERE `tutor`='none'";
...
<?php 
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `internalod` SET `tutor`='approved' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `internalod` SET `tutor`='declined' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
...
?>
		...
		<a href="tutorinternalod.php" id="gb"><button type="button" class="btn btn-secondary" 
