...
<body>
		...
		$dis="SELECT * FROM `medicalleave` WHERE `tutor`='none'";
...
<?php 
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `medicalleave` SET `tutor`='approved' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","college");
	$dis="UPDATE `medicalleave` SET `tutor`='declined' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
...
?>
		...
		<a href="leaveapplication.php" id="gb"><button type="button" 
		class="btn btn-secondary" data-bs-dismiss="modal" id="buti">Go Back</button></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="tutormedical.php" id="gb"><button type="button" class="btn btn-secondary" 
